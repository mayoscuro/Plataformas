...
public class RotatingPlatform : MonoBehaviour {
    ...
    /// </summary>
    private float m_LoopLimit = 90f; //180.0f;
	...
	void RotateAndStop()
	{
		...
		if (m_Stop)
		{
            stickyFloor.enabled = true;
            // keep original player rotation
	...
	void JustRotate()
	{
		if (m_RotateX)
		{
            ...
            if (m_CurrentXRotation % m_LoopLimit > 45)
            {
                stickyFloor.enabled = false;
		...
		if (m_RotateY)
		{
            ...
            if (m_CurrentYRotation % m_LoopLimit > 45)
            {
                stickyFloor.enabled = false;
                player.transform.rotation = playerRotation;
            }
		...
		if (m_RotateZ)
		{
            ...
            if (m_CurrentZRotation % m_LoopLimit > 45)
            {
                stickyFloor.enabled = false;
                player.transform.rotation = playerRotation;
            }
    ...
    private void OnTriggerStay(Collider other)
    {
        /*
        if (other.tag == "Player") {
            Collider cubeCollider = gameObject.GetComponent<Collider>();
            Debug.Log(transform.localRotation.eulerAngles.x);

            
            Vector3 rotation = Quaternion.Euler(transform.localRotation.eulerAngles.x, 0, 0) * Vector3.right;
            Debug.DrawLine(other.transform.position, rotation);
        }
        */
    }
}
